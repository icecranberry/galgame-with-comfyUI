...
在 v3.4.0 中，记忆系统升级为 **Memory v3 架构**，在三层结构之上增强了五项能力：

- **多重表示与索引**：同一段经历会同时生成事实陈述、情绪印象等多种表示，并建立实体与三元组索引，配合关键词、向量、实体四路混合检索——换一种说法也能被想起。
- **双时态记录**：每条记忆同时记录「事件发生在何时」与「何时被记下」，角色能分辨「上周刚聊到的」和「很久以前就定下的约定」。
- **@memory 主动回想**：角色不再只能被动等待系统注入记忆——当对话触及相关话题时，她可以主动发起检索，把相关的过去带进当前对话。
- **睡眠期整理**：在用户空闲时段，后台自动对记忆进行去重、合并与归档，并受模型调用预算约束，避免无节制消耗额度。
- **上下文预算**：注入给模型的动态信息（记忆、日程、天气等）按优先级分级降级，预算紧张时先舍弃低优先级内容，每次降级都有日志可查，不会静默截断。
...
界面采用自绘的 **Cel Glow 设计系统**——赛璐璐动漫质感 × 柔光玻璃氛围：贴纸般按下会下沉的按钮、糖果凹陷的输入框与下拉、硬阴影和波点纹理，配合细腻的入场与反馈动效。

内置 **暖色与暗夜两套主题**，在设置「功能开关」中一键切换，也可选择按时间自动切换。切换的不只是强调色——从聊天、朋友圈到背包、表情包管理和朋友圈分享海报，全部页面都会跟随主题联动：

| 主题 | 气质 |
|------|------|
| ☀️ 暖色 warm | 默认主题，珊瑚暖纸原色（v3.2 配色） |
| 🌙 暗夜 dark | 深色玻璃，夜间低亮度 |

主题偏好持久保存在设备本地；信箱、Toast 等页面保留独特的「暖纸」质感作为点缀，金色礼物等语义状态色不随主题变化，保证关键信息的辨识度。
...
项目已经具备角色管理、单聊、群聊、多配置组表情包、背包道具与每日宝箱、角色立绘、记忆、情绪、关系、朋友圈、相册、日程、奇遇、信箱、通知、多主题界面、桌面启动器和 Android 外壳等完整模块，能够作为实际应用持续使用和迭代。
...
| 界面主题 | 通常只有固定样式 | 暖色 / 暗夜 / 按时间三种模式，全界面联动 |
