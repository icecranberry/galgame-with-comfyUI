<template>
  <TownPaperPanel :open="open" :title="`${displayName}  ${isNpc ? '货摊' : '交易'}`" :busy="!!busy" :refreshing="loading"
    ...
    <div class="tnt-body">
      ...
      <template v-if="isNpc && stock">
        <div class="tnt-favor">
          <span>好感度</span>
          <b>{{ stock.favor }}</b>
          <span v-if="wallet" class="tnt-balance">我的邻币 <b>{{ wallet.available }}</b></span>
          <span class="tnt-muted">买下货品会随机提升 TA 对你的好感</span>
        </div>
        ...
        <div class="tnt-grid">
          <article v-for="good in stock.goods" :key="good.id" class="tnt-good" :class="{ 'is-sold': good.sold }">
            <div class="tnt-thumb">
              <img v-if="good.imageUrl" :src="good.imageUrl" :alt="good.title" />
              <span v-else class="tnt-thumb-pending">{{ good.imageStatus === 'failed' ? '无图' : '绘制中' }}</span>
            </div>
            <div class="tnt-good-body">
              <div class="tnt-good-head">
                <b>{{ good.title }}</b>
                <span class="tnt-price">{{ good.price }} 邻币</span>
              </div>
              <p class="tnt-desc">{{ good.description }}</p>
              <div class="tnt-good-foot">
                <span class="tnt-favor-chip">好感 +{{ good.favorDelta }}</span>
                <linshe-button variant="secondary" size="sm" :disabled="!!busy || loading || good.sold || !canAfford(good)"
                  :title="!canAfford(good) && !good.sold ? '可用邻币不够' : ''"
                  :loading="busy === good.id" @click="buy(good)">{{ good.sold ? '已售出' : (canAfford(good) ? '买下' : '邻币不够') }}</linshe-button>
              </div>
            </div>
          </article>
        </div>
      ...
      <template v-else-if="!isNpc && catalog">
        <section aria-labelledby="tnt-sells">
          ...
          <article v-for="spec in catalog.sells" :key="`s:${spec.templateId}`" class="tnt-row">
            ...
            <linshe-button variant="secondary" size="sm" :disabled="!!busy || loading || spec.price <= 0"
              :loading="busy === spec.templateId" @click="buyTemplate(spec)">买一件</linshe-button>
          </article>
        </section>
...
const catalog = ref(null), stock = ref(null)
...
async function buy(good) {
  ...
  try {
    ...
    if (current !== generation) return
    const favor = result.favor || {}
    const spent = result.money?.amount ?? good.price
    const delta = favor.delta ?? good.favorDelta
    notice.value = result.alreadyOwned
      ? `「${good.title}」已经在背包里了，这次没有重复扣款。`
      : `「${good.title}」已放进背包，花了 ${spent} 邻币，TA 对你的好感 +${delta}（从 ${favor.before ?? '?'} 涨到 ${favor.after ?? '?'}）`
    backpack.fetchItems().catch(() => {})
    emit('traded')
    await refresh()
  } catch (e) { if (current === generation) error.value = message(e) }
