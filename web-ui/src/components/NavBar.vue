<template>
  <nav class="nav-bar">
    <div class="nav-top" ref="navTopEl">
      <!-- 游戏菜单式滑动指示器：跟随 active 项滑动并点亮该项功能色 -->
      <div class="nav-indicator" :style="indicatorStyle" aria-hidden="true"></div>
      <router-link to="/chat" data-nav="chat" style="--hue: var(--fun-purple)" class="nav-item" :class="{ active: $route.path.startsWith('/chat') }" title="聊天">
      ...
      <div class="nav-item" data-nav="moments" style="--hue: var(--fun-pink)" :class="{ active: $route.path.startsWith('/moments') }" title="朋友圈" @click="handleMomentsClick">
        <div class="nav-icon-wrap">
      ...
      <router-link to="/tavern" data-nav="tavern" style="--hue: var(--fun-violet)" class="nav-item" :class="{ active: $route.path.startsWith('/tavern') }" title="酒馆">
        <div class="nav-icon-wrap">
          <svg viewBox="0 0 1024 1024" width="24" height="24" fill="currentColor">
...
function updateIndicator() {
  ...
  const item = wrap.querySelector(`[data-nav="${key}"]`)
  if (!item) { indicatorStyle.value = { opacity: 0 }; return }
  const hue = getComputedStyle(item).getPropertyValue('--hue').trim() || 'var(--fun-purple)'
...
watch(activeNavKey, () => nextTick(updateIndicator))
...
.nav-item:hover {
  ...
  color: color-mix(in srgb, var(--hue, var(--text-bright)) 72%, var(--text-bright));
}
