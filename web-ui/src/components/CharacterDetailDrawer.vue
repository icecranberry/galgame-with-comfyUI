<template>
  <Teleport to="body">
    <Transition name="drawer">
      <div v-if="open" class="drawer-overlay" @click.self="$emit('close')" @keydown.stop="onDrawerKeydown" @keyup.stop>
        <div ref="drawerPanel" class="drawer-panel" role="dialog" aria-modal="true" :aria-label="`${char?.display_name || '角色'}的日程`" tabindex="-1" @click.stop>
          <!-- 头部：分两行 -->
          <div class="dr-header">
            <!-- Row 1: 头像 + 信息 + 关闭 -->
            <div class="dr-row1">
            ...
            <!-- Row 2: 操作按钮 -->
            <div class="dr-row2">
              <linshe-button v-if="activities.length > 0" variant="secondary" class="dr-btn" @click="$emit('peek')">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></svg>
                <span>瞄一眼</span>
              </linshe-button>
              <linshe-button variant="secondary" class="dr-btn" @click="$emit('regenerate')" :disabled="regenerating">
              ...
              <template v-if="char?.is_sleeping && !char?.is_temp_woken">
                <!-- 被上门摇醒过 → 摇醒 -->
                <linshe-button v-if="char?.was_door_woken" variant="secondary" class="dr-btn" :class="{ shaking: doorShaking }" @click="onWakeDoor">
                ...
                <!-- 三次电话未叫醒 → 上门摇醒 -->
                <linshe-button v-else-if="(char?.wake_attempts || 0) >= 3" variant="secondary" class="dr-btn" :class="{ shaking: doorShaking }" @click="onWakeDoor">
                ...
                <!-- 默认 → 电话叫醒 -->
                <linshe-button v-else variant="secondary" class="dr-btn" :class="{ shaking: phoneShaking }" @click="onWakePhone">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" /></svg>
                  <span>电话叫醒</span>
                </linshe-button>
              </template>
              <!-- 正常 → 聊天按钮 -->
              <linshe-button v-else variant="secondary" class="dr-btn" @click="$emit('chat')">
              ...
              <div class="dr-scan-content">
                <div class="dr-scan-icon">
                  <svg viewBox="0 0 80 80" class="dr-scan-ring">
                    ...
                    <circle
cx="40" cy="40" r="34" fill="none" stroke="var(--accent)"
              ...
              >
                ...
                <div class="tl-content">
                  <div class="tl-top">
                    ...
                    <span v-if="act.edited" class="tl-edited" title="这条日程被悄悄改过，到点 ta 会发一条特殊朋友圈">改过啦</span>
                    <linshe-button variant="icon" size="sm" class="tl-peek-btn" title="编辑这条日程" @click.stop="openEdit(act, i)">
                    ...
                    <linshe-button variant="icon" size="sm" class="tl-peek-btn" title="瞄一眼这个瞬间" @click.stop="$emit('peekAt', act)">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></svg>
                    </linshe-button>
                  </div>
...
{{ tooltip.text }}
    ...
    <!-- 单条日程编辑弹窗 -->
    <linshe-modal v-model="editOpen" title="编辑日程">
      <div class="edit-field">
        <label class="edit-label">活动</label>
        <linshe-input v-model="editForm.activity" size="sm" placeholder="简短活动名" maxlength="60" />
      </div>
      <div class="edit-field">
        <label class="edit-label">地点</label>
        <linshe-input v-model="editForm.location" size="sm" placeholder="地点" maxlength="60" />
      </div>
      <div class="edit-field">
...
async function saveEdit() {
  if (editSaving.value || editIndex.value < 0 || !props.char) return
  if (editForm.activity.trim() === '') { notify('活动名不能为空', 'error'); return }
...
watch(() => props.open, async open => {
  if (open) {
    drawerReturnFocus = document.activeElement as HTMLElement
    await nextTick()
...
onUnmounted(() => { if (drawerReturnFocus?.isConnected) drawerReturnFocus.focus({ preventScroll: true }) })
function onDrawerKeydown(event: KeyboardEvent) {
  ...
  const nodes = [...drawerPanel.value.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), [tabindex="0"]')].filter(el => el.getClientRects().length)
  const first = nodes[0], last = nodes.at(-1)
  if (!first) { event.preventDefault(); drawerPanel.value.focus() }
...
.tl-top .tl-peek-btn + .tl-peek-btn { margin-left: 0; }
.tl-item:hover .tl-peek-btn { opacity: 0.35; }
.tl-peek-btn:hover { opacity: 1 !important; }
.tl-loc { font-size: 0.75rem; color: var(--text-secondary); margin-top: 3px; }

/* ── 单条日程编辑弹窗 ── */
.edit-hint {
  margin: 0 0 12px; font-size: 0.78rem; line-height: 1.5;
  color: var(--text-secondary);
}
