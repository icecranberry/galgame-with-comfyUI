...
进入世界页，顶栏可选 **HD2D / HD2D · 低配 / 兼容画面**。标准 HD2D 默认弱移轴，可通过统一开关关闭。偏好仅存在本机 localStorage；切换保留镜头、编辑草稿、移动状态和小镇订阅。WebGL2 创建失败、运行异常或 context lost 自动回退并显示提示；重新选择 HD2D 可重试。

标准档 DPR 上限 1.5、单盏方向投影灯、2048² 阴影；低配档 DPR 上限 1、方向阴影降到 1024²、关闭景深采样，保留阴影与色调合成。地砖按 16×16 块、素材和图层使用 InstancedMesh，编辑只重建改变的块。Three.js 锁定 **0.185.1**，按需加载独立 chunk。
