...
async function curateNow({ conversationId, throughRawMsgId, characterName = '', userName = '' }) {
  ...
  try {
    ...
    const prompt = buildMemoryCurationPrompt({ transcript, related });
...
export function buildMemoryCurationPrompt({ transcript, related = [] }) {
...
export async function curateAccumulatedMemory({ accumulatedText, characterName = '', userName = '用户' }) {
  ...
  const prompt = buildMemoryCurationPrompt({ transcript: accumulatedText, related: [] });
