...
export function getCoreDialogueRules({ userName, identityAnchor = true } = {}) {
  ...
  // ── 说人话（格式规则，所有聊天路径通用）──
  parts.push(`- 〖说人话〗你在聊天软件里发消息，不是写舞台剧本。严禁以下格式：①「（动作描述）台词」如"（靠近）你好""（压低声音）喂"；②第三人称旁白如"她停下脚步，凝视前方"。正确做法：像真人聊天一样直接说话，情绪通过台词、语气词和标点传达。如果真有动作要表达，用台词说出来，例如"你过来一下"而非"（招手）过来"。`);
