...
function detectNegation(text) {
  const negPatterns = /(?:一点也不|一点都不|并不|没有|不用|别这样|别|不|没)(?:\s*[的地得])?/;
...
const RULE_TABLE = [
  ...
  // ── 悲伤/失落 ──
  [/难过|伤心|哭|😢|😭|好想哭|想哭|泪|心碎|心痛|难受|好痛/i,
  ...
  // ── 害羞/腼腆 ──
  [/害羞|不好意思|难为情|脸红|羞涩|别[说讲]了|讨厌(?!厌)/i,
...
async function llmEvaluate(userMsg, assistantMsg, context = {}) {
  ...
  const prompt = `你是一个情绪与关系评估专家。你需要以角色的视角，分析一轮对话对角色产生的影响。

【角色人格】
${characterPersonality || '（未设定特殊人格，按默认友善助手判断）'}

【角色与用户的关系】
角色将用户视为：${relationship || '普通朋友'}
...
角色: "${assistantMsg.slice(0, 500)}"

【评估要求】
请以角色的视角（基于角色人格），分析本轮对话对角色产生的情绪影响和关系影响。
...
- reason: 用角色第一人称口吻简短解释判断理由（10~30字）

重要提示：
1. 角色的情绪基线是 valence=${emotionBaseline.valence?.toFixed(2) ?? '0.50'}, arousal=${emotionBaseline.arousal?.toFixed(2) ?? '0.50'}, dominance=${emotionBaseline.dominance?.toFixed(2) ?? '0.50'}，评估时以此为参考
2. 当前好感度越高，角色对用户的小善意越敏感（vad_delta 可适当放大）；好感度越低，越冷漠或防御
3. 亲和度变化 affinity_delta 应基于角色人格判断——例如傲娇角色即使内心高兴，好感度变化也较小
4. 如果用户消息中性平淡，情绪和好感度变化应该接近于 0，不要强行解读
5. 角色的最后一句话（assistant）是角色自己的回复，评估的重点是用户消息对角色情绪的影响，而非角色自己的回复内容
 ...
 */
export function emotionDashboard(state, dominantEmotion, affinity, affinityDelta, source) {
  const comp = getCompositeEmotion(state);
