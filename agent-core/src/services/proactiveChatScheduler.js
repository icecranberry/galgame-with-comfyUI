...
要求：
${proactiveRules}
- 自然口语化，像是突然想到就随手发了一条消息，不是写小作文；不要客套式收尾或总结陈词
- 以口语短句为主，可以省略主语、用半句话或语气词（如"就……""算了""在吗""突然想起来"），不要写成工整的书面句，也不要"首先/其次"这类条理分明的结构
- 可以拆成两三条连着发（用换行分隔，系统会自动切成多条消息），例如一条起头加一条补充；拆条后总量仍要落在这 15~50 字内
