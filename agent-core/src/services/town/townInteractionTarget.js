...
import { townCapabilities, defaultTownCapabilities, ensureTownCapabilities } from './townCapabilities.js';
...
export function resolveTownInteractionTarget(context, target) {
  ...
  const capabilities = building
    ? ensureTownCapabilities(db, 'town_locations', location, defaultTownCapabilities(location.business_kind))
    : npc ? ensureTownCapabilities(db, 'town_npcs', npc, defaultTownCapabilities(inferTownBusinessKind(npc.job), npc.job))
