...
import { buildChatContext, createPromptCacheKey, getCheckpointHistory, saveLlmContextSnapshot } from '../services/contextAssembler.js';
...
router.delete('/characters/:id/messages', (req, res, next) => {
  ...
  const doDelete = () => {
    ...
    db.prepare(`DELETE FROM llm_context_snapshots WHERE conversation_id = ?`).run(conversationId);
    db.prepare(`DELETE FROM llm_call_logs WHERE conversation_id = ?`).run(conversationId);
...
router.delete('/characters/:id/messages/last-round', (req, res, next) => {
  ...
  const doDelete = () => {
    ...
    db.pragma('foreign_keys = OFF');
    try {
      db.prepare(`DELETE FROM llm_context_snapshots WHERE conversation_id = ? AND user_raw_msg_id >= ?`)
...
- {"prompt":"Description of the scene"}：对话历史中若出现这种格式，意味着这里出现了一张这样的图片，继续自然对话即可。
    ...
    // ── checkpoint 历史 ──
    const { checkpoint, history } = getCheckpointHistory(db, conversationId);
    ...
    if (scheduleCtx) {
      if (activeEvent && activeEvent.emphasis_delivered) {
        const descSnippet = activeEvent.description.length > 50 ? activeEvent.description.slice(0, 50) + '...' : activeEvent.description;
        dynamicBlocks.push(`<schedule_context>\n${scheduleCtx.replace(/。$/, `。今天发生的「${activeEvent.title}」还在继续——${descSnippet}`)}\n</schedule_context>`);
      } else {
        dynamicBlocks.push(`<schedule_context>\n${scheduleCtx}\n</schedule_context>`);
      }
    } else if (activeEvent && activeEvent.emphasis_delivered) {
      const descSnippet = activeEvent.description.length > 50 ? activeEvent.description.slice(0, 50) + '...' : activeEvent.description;
      dynamicBlocks.push(`<schedule_context>\n【当前状态】今天发生的「${activeEvent.title}」还在继续——${descSnippet}\n</schedule_context>`);
    }

    // 角色视角的用户画像（原在 relParts / msgs[2]）
    ...
    // VAD 情绪状态 + 好感度（原在 relParts / msgs[2]）
    ...
    // 记忆三路召回（原在 materialParts / msgs[3]）
    ...
    // 最近奇遇总结（原在 materialParts / msgs[3]）
    ...
    // 重逢上下文 + streak 重置（原为独立注入到 msgs）
    const streak = getUnansweredStreak(characterId);
    if (streak >= 2) {
      dynamicBlocks.push(`【⚠️ 重逢提示 — 仅本次生成可见，不存入对话记录】assistant 之前连续发了 ${streak} 条主动消息 user 都没回——现在 user 终于回复了。assistant 应在接下来的回复中自然地流露一点"终于等到你"的情绪——不质问、不委屈、不阴阳怪气。嘴硬的用别扭的方式，温柔的用直接的方式，搞怪的用段子。让 user 感觉到：ta 回来聊天这件事，对 assistant 来说很重要。`);
    }
    if (streak > 0) {
      resetUnansweredStreak(characterId);
    }

    // 时间标签（原为直接 prepend 到 user 消息）
    const now = new Date();
    ...
    // ── 持久化上下文与本轮实际 RAG 注入快照 ──
    saveLlmContextSnapshot(db, {
      userRawMsgId,
      conversationId,
      characterId: parseInt(characterId, 10),
      summaryId: checkpoint?.id || null,
      checkpointEndMsgId: checkpoint?.end_msg_id || 0,
      memorySnapshot,
      dynamicContext: dynamicBlocks.filter(Boolean).join('\n\n'),
      promptRevision: metadata.revision,
      stablePrefixHash: metadata.stablePrefixHash,
      historyPrefixHash: metadata.historyPrefixHash,
    ...
    const streamOpts = {
      ...
      promptRevision: metadata.revision,
      requestHash: metadata.requestHash,
      cacheKey: createPromptCacheKey(conversationId, metadata.revision),
    };
...
async function judgeImageNeed(conversationId) {
  ...
  try {
    ...
    ], { temperature: 0, max_tokens: 5, label: '判断需要图片', requestKind: 'image_judge', conversationId, characterId: Number(conversationId.replace(/^char_/, '')) || null });
...
async function handleNeedImageFlow(conversationId, character, send, preExistingTaskId = null) {
  ...
  let fullContent = '';
  try {
    fullContent = await chatSync(msgs, { temperature: 0.7, max_tokens: 1024, label: '生图', requestKind: 'image_prompt', conversationId, characterId: character?.id ?? null });
...
输出：{"a":"好耶，我想吃火锅！","b":"不了吧，我们点外卖吃吃就好"}`);
  ...
  try {
    const result = await chatSync(msgs, { temperature: 0.7, max_tokens: 128, response_format: { type: 'json_object' }, label: '预测', requestKind: 'reply_guesses', conversationId, characterId: character?.id ?? null });
