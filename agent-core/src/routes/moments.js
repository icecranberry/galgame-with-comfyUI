...
async function generateMomentPost(character) {
  ...
  const STYLES = [
    ...
    { name: '养生/健康', desc: '泡脚、泡澡、早睡、养生茶、拉伸打卡，配图是养生场景或静谧氛围' },
    ...
    // === 格式/玩法向 ===
    { name: '二选一/投票', desc: '两个选项让朋友帮忙选——A还是B？哪件好看？吃哪个？配图是两选项的对比画面' },
    { name: '模仿/玩梗', desc: '模仿某个流行梗、句式或 format 发一条，语气玩味，配图是梗图风格或对应画面' },
    { name: '宣布/预告', desc: '宣布一个小决定、预告接下来要做的事，语气略带仪式感，配图是预告氛围' },
    // === 情绪向 ===
    { name: '摆烂/躺平', desc: '今天就是什么都不想干，理直气壮地摆烂，语气自嘲又解压，配图是瘫倒/懒散场景' },
    { name: '深夜放毒', desc: '深夜故意发美食，带着"反正你吃不到"的恶意卖萌，配图是诱人的食物特写（与普通美食的区别在于深夜+故意拉仇恨）' },
    { name: '谁懂啊', desc: '"谁懂啊！"开头的共鸣帖——吐槽或分享一种只有特定人群才懂的体验，寻求共鸣，配图是情绪化画面' },
    { name: '救命', desc: '"救命……"开头的轻度崩溃——被小事搞到抓狂但又很好笑，语气夸张但不真的严重，配图是搞笑崩溃表情/mood shot' },
    // === 叙事向 ===
    { name: '对比/前后', desc: '前后对比——整理房间、妆容变化、植物生长、身材变化，两帧画面的对比叙事，配图要体现变化' },
    { name: '陌生人观察', desc: '在公共场所观察到的有趣陌生人或场景片段，像一篇微型非虚构写作，配图是场景氛围' },
    { name: '翻车现场', desc: '尝试做某事的翻车现场——烘焙失败、发型翻车、手残惨剧，语气自嘲好笑，配图是翻车成果' },
    { name: '偶遇/巧合', desc: '讲述一个巧合或意外的偶遇，宇宙的安排感，配图是偶遇场景或暗示命运的 mood shot' },
    // === 展示向 ===
    { name: '植物/花草', desc: '养的植物开花了、新叶子展开了、多肉长胖了，语气像在养宠物，配图是植物特写' },
    // === 互动向 ===
    { name: '有奖竞猜', desc: '发起一个有趣的小竞猜或解谜，让朋友圈猜答案，配图是谜面或线索' },
    { name: '今日人设', desc: '"今天的人设是____"——给自己一个临时人设标签，然后围绕它展开今天的行事风格，配图是人设氛围' },
    { name: '求助后续', desc: '上次求助/吐槽的后续更新，感谢大家的建议，交代事情进展，配图是结局氛围' },
  ];
...
规则：
- 只输出 JSON，不要解释
- **世界观驱动**：你的朋友圈发生在上述世界观中，不是在真空或现实世界中。你分享的日常、你的语气、你描述的场景和互动方式，都应该是这个世界里一个普通人发的朋友圈——这个世界的"日常"就是你的日常，不需要刻意解释。
- 文案和配图 prompt 必须语义一致
