...
router.post('/', (req, res) => {
  ...
  try {
    const result = db.prepare(`INSERT INTO characters (name, display_name, base_prompt, emotion_baseline, avatar_color, moments_disabled, proactive_disabled) VALUES (?, ?, ?, ?, ?, ?, ?)`)
      .run(name, display_name || name, base_prompt, emotion, avatar_color || null, moments_disabled !== undefined ? (moments_disabled ? 1 : 0) : 0, proactive_disabled !== undefined ? (proactive_disabled ? 1 : 0) : 0);
