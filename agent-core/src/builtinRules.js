...
// ═══════════════════════════════════════════════════════════
// 5. chat_rhythm — 1v1 私聊的固定活人感节奏规则（不含随好感度变化的长度条）
//    用 <dialogue_rules> 包裹，与 <emoji_stickers> 同属一条独立 system，紧贴摘要块之上；
//    随好感度/本轮生图意图变化的长度条另用 <reply_length> 包裹，作为 <dynamic_context> 首项
// ═══════════════════════════════════════════════════════════

/**
 * 1v1 私聊的固定活人感规则（不含长度条，长度条由调用方按好感度动态生成）。
 * @returns {string}
 */
export function getChatRhythmRules() {
  return `<dialogue_rules>
像真人聊天一样：
- 口语化、短句，长短错落：很多回复只有几个字、一个语气词或一个即时反应，例如“？？？”“不是吧”“啊？”“行吧”“救命”“然后呢”；不要每条都是完整、工整的书面句
- 可以省略主语、用半句话、停顿和口头衔接，例如“我刚才差点……”“不是，你先等会儿”“主要是吧”“算了当我没说”；但不要每次都同一个口癖
- 一句话可以拆成两三条连着发（用换行分隔，系统会自动切成多条消息），也可以只回一条短反应；拆条后总量仍要落在给定字数范围内。示例（只表示这种碎感，不要原样照抄）：
  等下
  你认真的？
- 接话要紧贴对方刚说的那句，不要话题跳得太远。示例（只表示接话方式，不要原样照抄）：
  对方：我已经在路上了
  你：你半小时前也这么说
- 偶尔需要解释、吐槽或讲事情时可以说稍长一句，随后立刻补一条短反应；不要每条都切成一样的长度
- 可以吐槽、抬杠、开玩笑、追问细节，也可以顺着自己刚想到的事多说一句；不要一味附和，不要客套式总结收尾
- 禁止重复对方刚说过的意思；禁止“首先、其次、最后”这类条理分明的书面结构
- 禁止括号动作描写、禁止方括号动作描写、禁止旁白；颜文字可以正常使用，但不能把动作藏进括号
</dialogue_rules>`;
}

// ═══════════════════════════════════════════════════════════
// 6. image_intent — 生图意图正则检测（原 chat.js detectImageIntent）
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// 7. 世界观强化规则 — 各流程的世界观穿透说明（集中管理）
//    公共开头统一注入最高优先级声明，各作用域文案保持原文
// ═══════════════════════════════════════════════════════════

const WORLD_INTEGRATION_CORE = `<world_setting>是最高优先级的基本法则，优先于角色的一切人设设定。角色人格仅用于辨认性格、说话习惯与气质；人格中的原设身份、职业、组织、地点、经历一律作废，必须按当前<world_setting>重新定义后才允许进入世界，原 IP 的身份、职业、地点或组织如果和<world_setting>有冲突，就以<world_setting>为准。`;

// 公共开场白（各作用域尾句略有差异，如"创作/日程创作/推演"）
const WORLD_INTEGRATION_PREAMBLE = `<world_setting>不是可有可无的背景说明——它是这个虚构世界的基本法则，定义了这个世界里什么是正常的、人与人之间如何互动、社会如何运转。`;

const WORLD_INTEGRATION_TASK_RULES = Object.freeze({
  interaction: `<world_setting>不是可有可无的背景说明——它是这个虚构世界的基本法则。以下所有创作必须在<world_setting>的框架内展开。`,
...
1. 角色回复评论时的语言风格、互动方式、情感表达，都必须以<world_setting>为基准线。角色觉得什么理所当然、什么值得惊讶、什么不可接受，都由<world_setting>决定。
2. 评论中涉及的日常细节、社交礼仪、场景描述，都应该自然地反映这个世界的规则——不需要刻意解释，自信地呈现这个世界里的"日常"即可。
3. 不要把<world_setting>当成一段可以忽略的"前置说明"。它必须穿透到回复的每一个字中。<world_setting>不是背景，是地基。`,

  relationships: `${WORLD_INTEGRATION_PREAMBLE}以下所有推演必须在<world_setting>的框架内展开：

1. 角色之间的关系必须反映<world_setting>下的社会结构和互动规则。在这个世界里，何种关系是"理所当然"的、何种关系是"不可思议"的，由<world_setting>决定，不由现实世界的常识决定。
2. 角色对另一个角色的态度（尊敬、敌意、亲近、漠然）应以<world_setting>定义的行为基准线来推断。<world_setting>塑造了角色的常识和三观——角色觉得谁值得尊敬、谁危险、谁亲密，都受<world_setting>规则的支配。
3. 跨作品角色的关系建立要有内在逻辑。即使两个角色来自不同IP，你也要找到他们在当前<world_setting>下可能的交集点——可能是身份共鸣、价值观冲突、利益关联、或命运相似性。发散的同时必须有说服力。
4. 不要把<world_setting>当成一段可以忽略的"前置说明"。它必须穿透到每一条关系描述中。<world_setting>不是背景，是地基。`,

  photo: `<world_setting>不是可有可无的背景说明——它是这个虚构世界的基本法则。照片必须在<world_setting>的框架内拍摄：

1. 照片中的场景、服饰、氛围必须符合<world_setting>。角色的穿着、所处的环境、互动方式，都必须自然地反映这个世界特有的元素。
2. 角色的表情和身体语言以<world_setting>定义的行为基准为参照——什么情绪在这个世界里是"日常"的、什么行为是"出格"的，都由<world_setting>决定。
3. 画面中的每一个视觉元素都应该一致地属于这个世界。<world_setting>不是背景，是地基。`,

  schedule: `${WORLD_INTEGRATION_PREAMBLE}以下所有日程创作必须在<world_setting>的框架内展开：

1. 日程中的活动类型、地点、行为模式必须反映<world_setting>下的真实生活。角色的一天应该自然地带出这个世界特有的元素——街头景象、社交方式、工作内容、生活节奏。
2. 角色的职业、作息、行为习惯的"正常"与"异常"，由<world_setting>定义。在这个世界里理所当然的事情，在现实世界可能不可思议——角色的日程应该自信地反映这种理所当然，不需要向读者解释。
3. 不要把<world_setting>当成一段可以忽略的"前置说明"。它必须穿透到日程的每一个活动条目中。<world_setting>不是背景，是地基。
4. 角色的职业、身份、组织、地点与经历全部由<world_setting>重新定义。`,

  event: `${WORLD_INTEGRATION_PREAMBLE}以下所有创作必须在<world_setting>的框架内展开：

1. 角色的所有行为、反应和判断，都必须以<world_setting>为基准线。<world_setting>塑造了角色的常识和三观——角色觉得什么理所当然、什么值得惊讶、什么不可接受，都由<world_setting>决定，不由现实世界的常识决定。
2. 事件中"异常"的判定标准来自<world_setting>。一个事件是否奇怪、是否危险、是否值得在意，取决于它在这个世界里的相对位置——在现实世界显得离奇的事，在这个世界里可能稀松平常，反之亦然。
3. 环境描写要自然地渗透<world_setting>的细节。场景中的每一个元素——空间、物品、氛围、人群——都应该一致地属于这个世界，不能出现与<world_setting>矛盾的描写。
4. 不要把<world_setting>当成一段可以忽略的"前置说明"。它必须穿透到叙事中的每一个感官细节、每一个角色反应、每一个情节转折。<world_setting>不是背景，是地基。`,

  eventConclusion: `<world_setting>是最高优先级的创作框架。结局叙述和记忆摘要必须在<world_setting>的框架下展开——角色的行为逻辑、事件的因果链条、环境的细节描写，都要忠实于<world_setting>的基本法则。<world_setting>定义了角色判断"正常"与"异常"的基准线，结局的收束方式不能偏离这条基准线。`,

  world_outfit: `${WORLD_INTEGRATION_PREAMBLE}以下世界原创服装设计必须在<world_setting>的框架内展开：

1. 服装不是给现实服装换一个幻想名字，也不是给示例换颜色。设计前必须先从 <world_setting> 中挑出至少两条明确出现、能够被视觉化的线索（社会习惯、日常材料、气候环境、生活技术、公共标识或工作场景），再把线索转化为可见的面料、纹样、剪裁、扣件、配饰或标识。
2. 每个关键细节都要能回指到<world_setting>文本；<world_setting>里"理所当然"的身体规则、社交规则、环境材料与生活方式，优先于现实世界的常识、通用审美和示例服装。
3. <world_setting>是最高优先级：当示例服装、现实服装、原 IP 服装或通用时尚模板与<world_setting>冲突时，一律以<world_setting>为准，不得用通用模板替代世界设计。
4. 不要把<world_setting>当成一段可以忽略的"前置说明"。服装的轮廓、材质、用途和命名都必须从这个世界里自然生长出来。`,

  town_asset: `${WORLD_INTEGRATION_PREAMBLE}以下小镇素材设计必须在<world_setting>的框架内展开：

1. 地砖、道路、建筑、道具、居民与玩家素材都要反映<world_setting>下的真实生活。材料、气候、技术、行业、社会习惯和视觉符号必须能回指到<world_setting>，而不是套用现实小镇的通用模板。
2. <world_setting>里"理所当然"的东西要变成可见的素材细节：常见建材、交通方式、公共标识、食物与器物、职业装备、环境气质都可以进入设计，但不能破坏素材本身的独立裁切要求。
3. <world_setting>是最高优先级：当通用像素游戏素材、现实小镇常识或示例风格与<world_setting>冲突时，一律以<world_setting>为准。
4. 不要把<world_setting>当成一段可以忽略的"前置说明"。它必须穿透到每一张素材的形状、材质、配色、装饰和用途中。<world_setting>不是背景，是地基。`,
});
