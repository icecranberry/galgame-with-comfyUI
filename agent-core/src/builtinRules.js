...
const WORLD_INTEGRATION_CORE = `<world_setting>是最高优先级的基本法则，优先于角色的一切人设设定。角色人格仅用于辨认性格、说话习惯与气质；人格中的原设身份、职业、组织、地点、经历一律作废，必须按当前世界观重新定义后才允许进入世界，原 IP 的身份、职业、地点或组织如果和世界观有冲突，就以世界观为准。`;
...
4. 不要把世界观当成一段可以忽略的"前置说明"。它必须穿透到叙事中的每一个感官细节、每一个角色反应、每一个情节转折。世界观不是背景，是地基。`,
  ...
  eventConclusion: `<world_setting>是最高优先级的创作框架。结局叙述和记忆摘要必须在<world_setting>的框架下展开——角色的行为逻辑、事件的因果链条、环境的细节描写，都要忠实于世界观的基本法则。世界观定义了角色判断"正常"与"异常"的基准线，结局的收束方式不能偏离这条基准线。`,
