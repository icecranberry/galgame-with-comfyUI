...
test('真实生产运行层：配送消费后两位居民持续工作、补一份库存并支付公共费用', async t => {
  ...
  // Advance actual scheduler observations: offline wall time alone must not manufacture a proof.
  let produced = false;
  for (let minute = 0; minute < 40; minute++) {
    now += 60000;
    town.forceTick();
    if (economy.getTownEconomyState().production.batches.some(batch => batch.status === 'completed')) { produced = true; break; }
  }
  assert.equal(produced, true);
  const runtime = economy.getTownBusinessRuntime();
  const slice = runtime.business.getSlice(runtime.scope);
  assert.equal(runtime.economy.getStock({ ...runtime.scope, stockId: slice.stocks.supplier }).quantity, 20);
  assert.equal(runtime.economy.getAccount({ ...runtime.scope, accountId: slice.accounts.fund }).balance, 1990);
  assert.equal(runtime.economy.getAccount({ ...runtime.scope, accountId: slice.accounts.workshop }).balance, 400);
  assert.equal(runtime.production.getResourceNode(runtime.scope).remaining, 199);
  assert.equal(db.prepare('SELECT count(*) n FROM town_production_proofs').get().n, 2);
