...
>
> 文档状态：调研证据保留；2026-09-07 已落实 **M1 画面渲染实现及桌面隔离验收**，详见 [M1 实施记录](docs/hd2d-m1.md)。Android 真机与真实存档人工验收仍待进行；M0 其余业务契约、M2–M7 未在本轮实施。以下“本次调研”中的隔离限制描述的是原调研阶段。
>
...
天气事实变雨 → 本地规则降低户外访问、提高室内访问倾向 → 咖啡馆客流上升、原料库存下降 → 发布有预算的配送单 → 玩家在信箱任务卡接受 → 去生产者领取并送达 → 原子交货发工资 → 饮品服务恢复供应 → 玩家可花刚赚的钱体验调制。若与已关联角色一起完成，角色可以在朋友圈说“今天的热饮是你帮忙送来的”；后续聊天记得这次帮助。
...
场景只选公告站、原料点、工坊三处，3 个职业 actor，保留已有入住角色。以下整条链必须可以在同一存档重复正常运行：

1. 玩家在 HD2D 街道移动，建筑/人物投影方向一致，远景轻微失焦。
2. 在公告站接一份已预留 30 邻币预算的材料配送单。
3. NPC 根据本地规则把原料准备好；玩家到达领取，去工坊真实交付。
4. 工坊收货；玩家收到 30 邻币，账本和原料变更一次，日志解释来龙去脉。
5. 玩家花 30 邻币参加工坊主题服务，立绘 + 游戏对白交互，服务自然或硬条件结束。
6. 产出一个映射已有 effect 的道具并收入原背包；退出/失败时按既定政策退款。
7. 玩家在原背包使用道具，已有角色效果生效；一条重要经历进入记忆，后续普通聊天可自然提及。
8. 断网重连、服务中重启、重复提交不会多扣钱、多发物品或让 NPC 永久忙碌。
