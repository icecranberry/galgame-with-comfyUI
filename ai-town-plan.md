...
> 实测已验证：蓝图/作息/相遇对话/状态气泡/就地聊天 LLM 调用、ComfyUI 全套素材生成（地砖/道路/建筑/道具/四方向精灵）、开镇后 NPC 按作息昼夜流动、点击寻路、编辑器。
...
> **实施补记（等距美术口径）**：地砖/道路不带画师串（illustration 画师会把平铺纹理带偏），styleTags 中的聚落类名词（village 等）会被剥除防止「草地上长出小房子」；地面/道路用 promptTemplate 把素材描述插进「顶面完全被该材质覆盖」句式（否则模型画成花坛）；建筑/道具用正向措辞「sits directly on the background with a clean straight bottom edge, no base platform」去除底座砖。后处理 `flattenIsoTile` 把等距地砖裁出顶面菱形 + alpha 蒙版归一化成 64×32 标准贴图（接缝完美互锁，菱形位置漂移由 `detectTileAnchorY` 最宽行检测兜底）；素材 meta 记录 `updatedAt` 供前端 URL 缓存穿透。
